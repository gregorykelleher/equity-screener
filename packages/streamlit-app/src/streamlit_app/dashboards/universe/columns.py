...
COLUMN_GROUPS: tuple[ColumnGroup, ...] = (
    ...
    ColumnGroup("Market",         ("Last Price", "Market Cap", "52W Min", "52W Max",
                                   "Market Volume")),
    ColumnGroup("Ownership", (
        "Held Insiders", "Held Institutions", "Short Interest",
        "Share Float", "Shares Outstanding",
    )),
    ColumnGroup("Valuation",      ("Trailing P/E", "Price/Book", "Trailing EPS",
                                   "Revenue/Share", "Dividend Yield")),
    ColumnGroup("Profitability",  ("Profit Margin", "Gross Margin", "Operating Margin",
                                   "ROE", "ROA")),
    ColumnGroup("Financials",     ("Revenue", "EBITDA", "Free Cash Flow",
...
CURRENCY_COLS: frozenset[str] = frozenset({
    "Last Price", "52W Min", "52W Max", "Revenue/Share", "Trailing EPS",
})
LARGE_CURRENCY_COLS: frozenset[str] = frozenset({
    "Market Cap", "Free Cash Flow", "Operating Cash Flow",
    "Total Debt", "Revenue", "EBITDA",
})
PERCENTAGE_COLS: frozenset[str] = frozenset({
    "Dividend Yield", "Held Insiders", "Held Institutions", "Short Interest",
    "Profit Margin", "Gross Margin", "Operating Margin", "ROE", "ROA",
    "1Y Performance",
})
LARGE_NUMBER_COLS: frozenset[str] = frozenset({
    "Market Volume", "Share Float", "Shares Outstanding",
})
...
PCT_STYLE_COLS: frozenset[str] = frozenset({
    ...
    "ROE", "ROA", "1Y Performance", "Dividend Yield",
})

IDENTIFIER_COLS: frozenset[str] = frozenset({
    "Share Class FIGI", "ISIN", "CUSIP", "CIK", "LEI", "MICs", "Currency",
})

_NUMBER_FILTER = {"filter": "agNumberColumnFilter"}

FORMATTER_TABLE: tuple[FormatterRule, ...] = (
    FormatterRule(CURRENCY_FMT,       CURRENCY_COLS,       _NUMBER_FILTER),
    FormatterRule(LARGE_CURRENCY_FMT, LARGE_CURRENCY_COLS, _NUMBER_FILTER),
    FormatterRule(PERCENTAGE_FMT,     PERCENTAGE_COLS,     _NUMBER_FILTER),
    FormatterRule(LARGE_NUMBER_FMT,   LARGE_NUMBER_COLS,   _NUMBER_FILTER),
    FormatterRule(RATIO_FMT,          RATIO_COLS,          _NUMBER_FILTER),
)
