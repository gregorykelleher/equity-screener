...
def get_cached_grid_options(df: pd.DataFrame) -> dict[str, Any]:
    """
    Return grid options, building and caching them in session state on first call.

    The options are built once per session since the column structure is static.

    Args:
        df: Source DataFrame whose columns seed the GridOptionsBuilder.

    Returns:
        Complete grid options dict ready for AgGrid rendering.
    """
...
def _build_grid_options(df: pd.DataFrame) -> dict[str, Any]:
    ...
    Runs the column configuration, sets tooltip config, and applies grouped
    column definitions.
...
def _build_grouped_column_defs(flat_col_defs: list[dict]) -> list[dict]:
    ...
    Returns:
        Grouped column definition list with headerName/children structure.
    """
    ...
    for cd in flat_col_defs:
        if cd.get("field") and cd["field"] not in grouped_fields:
            grouped.append(cd)
